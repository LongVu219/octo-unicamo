"""
Config for training Octo-small from scratch on BridgeV2 5K subset.
...
Usage:
    ...
    python scripts/finetune.py \
        --config scripts/configs/finetune_bridge5k_config.py:full,language_conditioned \
        --config.pretrained_path=hf://rail-berkeley/octo-small \
        --config.save_dir=checkpoints/bridge5k_from_scratch \
        --config.wandb.project=octo_bridge5k \
...
def get_config(config_string="full,language_conditioned"):
    ...
    # Our 5K dataset config
    FINETUNING_KWARGS = {
        "name": "bridge5k_dataset",
        ...
        "standardize_fn": "octo/data/oxe/oxe_standardization_transforms.py:bridge5k_dataset_transform",
    }
    ...
    config = dict(
        ...
        seed=42,
        wandb=dict(
            project="octo_bridge5k", group=placeholder(str), entity=placeholder(str)
        ),
