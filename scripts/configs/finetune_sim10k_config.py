"""
Config for finetuning pretrained Octo-small on synthetic 10K subset (sim_5k_1 + sim_5k_2).
...
Usage:
    ...
    python scripts/finetune.py \
        ...
        --config.save_dir=checkpoints/sim10k_finetune \
        --config.wandb.project=octo_sim10k \
...
def get_config(config_string="full,language_conditioned"):
    ...
    FINETUNING_KWARGS = {
        ...
        "standardize_fn": "octo/data/oxe/oxe_standardization_transforms.py:sim10k_dataset_transform",
    }
    ...
    config = dict(
        ...
        seed=42,
        wandb=dict(
            project="octo_sim10k", group=placeholder(str), entity=placeholder(str)
        ),
        dataset_kwargs=FINETUNING_KWARGS,
        modality=task,
        finetuning_mode=mode,
        window_size=window_size,
        skip_merge_pretrained=False,
        optimizer=dict(
            learning_rate=dict(
                name="cosine",
                init_value=0.0,
