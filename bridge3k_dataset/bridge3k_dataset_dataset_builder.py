"""
TFDS/RLDS dataset builder for the BridgeV2 3K subset.

Reads from our extracted real_3k_1 format (parquet + individual mp4 per episode)
and produces an RLDS dataset compatible with Octo's data pipeline.

The output matches the original bridge_dataset RLDS schema so we can reuse
Octo's existing bridge_dataset_transform and OXE config.
...
Usage:
    ...
    python -m tensorflow_datasets.scripts.document_datasets \
        --datasets=bridge3k_dataset \
        --module_import=bridge3k_dataset
    # Or just run convert_to_rlds.py which handles building.
"""
...
_DESCRIPTION = "BridgeV2 3K episode subset for Octo training from scratch."
...
class Builder(tfds.core.GeneratorBasedBuilder):
    """TFDS builder for BridgeV2 3K subset."""

    VERSION = tfds.core.Version("1.0.0")
    RELEASE_NOTES = {"1.0.0": "Initial 3K subset."}
    ...
    def _generate_examples(self):
        ...
        for ep_idx in sorted(df["episode_index"].unique()):
            ...
            # Handle length mismatch: use min of video frames and parquet rows
